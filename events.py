    ...
    try:
        if 'user' in session:
            ...
            if request.method=='POST' and 'create' in request.form:
                ...
                create_events(event_name,date,time,place,cust,phone)
            ...
            # if edit is clicked and then update is made data is update4d in row and database
                ...
                update_events(id,event_name,date,time,place,cust,phone)
                return redirect('/events')
            
            # function to display all the events data in table
            list_data=events_list()
            return render_template('events.html',list_data=list_data,edit_data=edit_data)
